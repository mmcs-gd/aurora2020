...
export default function buildLevel(width, height, maxRooms, scene) {
    ...
    const outsideLayer = scene.map.createBlankLayer("Outside", tileSet);  // createBlankDynamicLayer
    ...
    const { rooms, mask } = levelGenerator.generateMask();
	...
	for (let x = 0; x < width; x++) {
		for (let y = 0; y < height; y++) {
			if (mask[x][y] == 0) {
                ...
                groundLayer.weightedRandomize(x, y, 32, 32, LEVEL_TO_TILE[1]); // TILES.FLOOR
			}
		}
    }
    ...
    // добавляем персонажа Аврору в сцену
