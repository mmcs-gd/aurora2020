...
export default class Scene{
    ...
    generateScene(){
        ...
        this.SceneMatrix = this.generateEmptyScene(); 
        const startCenters = []; 
    ...
    expanding(room, isHorizontal){
        ...
        for(let y = room.top; y <= room.down; y++)
            for(let x = room.left; x <= room.right; x++){
                attachedPoints = attachedPoints.concat(attachMethod(x, y, room.id, this.SceneMatrix));
            }
        attachedPoints.forEach(p => {
            ...
            this.SceneMatrix[p.y][p.x] = room.id;
        })
    ...
    horAttach(x, y, id, Scene){
        const attached = [];
        if(x-1 >= 0 && Scene[y][x] === id && Scene[y][x-1] === 0)
            attached.push(new Vector2(x-1, y));
        if(x+1 < Scene[0].length && Scene[y][x] === id && Scene[y][x+1] === 0)
            attached.push(new Vector2(x+1, y));
        return attached;
    }

    vertAttach(x, y, id, Scene){
        const attached = [];
        if(y-1 >= 0 && Scene[y][x] === id && Scene[y-1][x] === 0)
            attached.push(new Vector2(x, y-1));
        if(y+1 < Scene.length && Scene[y][x] === id && Scene[y+1][x] === 0)
            attached.push(new Vector2(x, y+1));
        return attached;
    }
    ...
    generateEmptyScene(){ 
        // начальное заполнение матрицы уровней
        const emptyScene = [];
